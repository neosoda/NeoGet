...
public class AppStatusDiscoveryServiceTests
{
    ...
    // --- InstalledBinaryHint stamping (integration tests — require Windows registry) ---

    [Fact]
    public async Task GetInstallationStatusBatch_StampsInstalledBinaryHint_FromRegistryDisplayIcon()
    {
        // This test relies on a real registry write under HKCU\Software\Microsoft\Windows\
        // CurrentVersion\Uninstall — wrap in a try/finally that cleans up.
        const string subKey = @"Software\Microsoft\Windows\CurrentVersion\Uninstall\WinhanceTest_App";
        using var key = Microsoft.Win32.Registry.CurrentUser.CreateSubKey(subKey);
        try
        {
            key.SetValue("DisplayName", "Winhance Test App");
            key.SetValue("DisplayIcon", "C:\\TestPath\\TestApp.exe,0");

            var def = new ItemDefinition
            {
                Id = "test-app-binary-hint",
                Name = "Winhance Test App",
                Description = "Test fixture",
                RegistryDisplayName = "Winhance Test App",
            };

            var result = await _service.GetExternalAppsInstallationStatusAsync(new[] { def });

            result[def.Id].Should().BeTrue();
            def.InstalledBinaryHint.Should().Be("C:\\TestPath\\TestApp.exe");
        }
        finally
        {
    ...
    [Fact]
    public async Task GetInstallationStatusBatch_StampsInstalledBinaryHint_FromDetectionPathsMatch()
    {
        var tempFile = Path.Combine(Path.GetTempPath(), "WinhanceTest_" + Path.GetRandomFileName() + ".exe");
        File.WriteAllBytes(tempFile, new byte[] { 0x4D, 0x5A }); // MZ header
        try
        {
            var def = new ItemDefinition
            {
                Id = "test-app-detection",
                Name = "Detection Test",
                Description = "Test fixture",
            ...
            result[def.Id].Should().BeTrue();
            def.InstalledBinaryHint.Should().Be(tempFile);
        }
        finally
        {
